...
validation_builder = SingleViewTripletBuilder(arguments.validation_directory, IMAGE_SIZE, arguments, sample_size=2)
...
def main():
    ...
    ITERATE_OVER_TRIPLETS = 7
    tripletdir="./data/train_triplets/"
    for epoch in tqdm(range(arguments.start_epoch, arguments.start_epoch + arguments.epochs)):
        logger.info("Starting epoch: {0} learning rate: {1}".format(epoch,
            learning_rate_scheduler.get_lr()))
        filenames=[p for p in os.listdir(tripletdir) if p[0] != '.']
        triplet_data = [os.path.join(tripletdir, f) for f in filenames]
        #validate(tcn, use_cuda, arguments)
        epochloss=[]
        for dataset in triplet_data:
            ...
            epochloss.append(np.mean(losses))
